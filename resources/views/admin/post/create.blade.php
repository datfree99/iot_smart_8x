...
@section("content")
    ...
    <div>
        {!! Form::open(['method' => 'post', 'url' => route('admin.post.store')]) !!}
        <div class="row">
            <div class="col-md-4">
                ...
                <div class="form-group">
                    ...
                    {!! Form::textarea('description', old('description'), ['class' => 'form-control', 'placeholder' => 'Nhập mô tả...', 'rows' => 3]) !!}
                ...
                <div class="form-group">
                    ...
                    <select name="category" class="form-control">
                        ...
                        @foreach($categories as $category)
                            <option value="{{$category->id}}"  @if($category->id == old('category')) selected @endif>{{$category->name_en}}</option>
                            @if(isset($category->children))
                                @foreach($category->children as $subCateLv1)
                                    <option value="{{$subCateLv1->id}}" @if($subCateLv1->id == old('category')) selected @endif>
                                        -- {{$subCateLv1->name_en}}
                                    </option>
                                    @if(isset($subCateLv1->children))
                                        @foreach($subCateLv1->children as $subCateLv2)
                                            <option value="{{$subCateLv2->id}}" @if($subCateLv2->id == old('category')) selected @endif>
                                                --- {{$subCateLv2->name_en}}
                                            </option>
                                        @endforeach
                                    @endif
                ...
                <div class="form-group">
                    ...
                    <div>
                        <img src="{{old('image') ?? '/assets/images/default.png'}}" alt="Ảnh bài viết" id="btn_file_add" class="image-preview">
                ...
                <div class="form-group">
                    ...
                    {!! Form::text('seo_title', old('seo_title'), ['class' => 'form-control', 'placeholder' => 'Nhập seo tiêu đề...']) !!}
                </div>
                <div class="form-group">
                    {!! Form::label('seo_description', 'Seo mô tả', [], false) !!}
                    {!! Form::textarea('seo_description', old('seo_description'), ['class' => 'form-control', 'placeholder' => 'Nhập seo mô tả...', 'rows' => 3]) !!}
                </div>
                <div class="form-group">
                    {!! Form::label('seo_keywords', 'Seo keywords', [], false) !!}
                    {!! Form::textarea('seo_keywords', old('seo_keywords'), ['class' => 'form-control', 'placeholder' => 'Nhập seo keywords...', 'rows' => 3]) !!}
                </div>
            </div>
            <div class="col-md-8">
                <label for="">Nội dung bài viết <span class="required-label">(*)</span></label>
                <textarea name="contents" id="content-ck" class="form-control" rows="20" placeholder="Nhập mô tả">{!! old('contents') !!}</textarea>
            </div>
        </div>
